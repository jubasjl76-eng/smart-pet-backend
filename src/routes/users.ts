...
router.post(
  ...
  apiRoute({
    ...
    tags: T,
    secure: true,
    summary: 'Invite a user (returns a token; email delivery is Phase 3).',
    request: {
      body: z.object({ email: z.string().optional(), role: z.enum(['owner', 'staff']).optional() }),
    },
    responses: { 201: { description: 'created' }, 400: { description: 'invalid email' } },
  }),
  async (req: AuthRequest, res: Response) => {
...
router.patch(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    const nextRole =
      req.body?.role === 'owner' || req.body?.role === 'staff' ? req.body.role : target.role;
