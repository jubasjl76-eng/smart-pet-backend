...
router.post(
  ...
  apiRoute({
    method: 'post',
    path: '/api/breeder/ops/devices/pairing',
    tags: T,
    secure: true,
...
router.get(
  ...
  apiRoute({
    ...
    responses: {
      200: {
        description: 'ok',
        schema: z.object({ pairings: z.array(z.record(z.string(), z.unknown())) }),
      },
    },
  }),
  ah(async (req, res) => {
...
router.delete(
  ...
  apiRoute({
    method: 'delete',
    path: '/api/breeder/ops/devices/pairing/{code}',
    tags: T,
    secure: true,
...
router.post(
  ...
  apiRoute({
    method: 'post',
    path: '/api/breeder/ops/devices/claim',
    tags: T,
    secure: true,
...
// POST /api/breeder/ops/devices/:deviceId/rotate-credentials (Phase 21, A12 #20)
// — a fresh MQTT password for an already-claimed device, no pairing code
// needed. Pushes the new password to the device over MQTT if it's online;
// returns it too, for the admin's own records or a manual fallback.
router.post(
  '/:deviceId/rotate-credentials',
  apiRoute({
    method: 'post',
    path: '/api/breeder/ops/devices/{deviceId}/rotate-credentials',
    tags: T,
    secure: true,
    summary: 'Rotate MQTT credentials for an already-claimed device.',
    request: { params: z.object({ deviceId: z.string().min(1) }) },
    responses: { 200: { description: 'rotated' }, 404: { description: 'not found' } },
  }),
  ah(async (req, res) => {
    try {
      const result = await rotateDeviceCredentials(String(req.params.deviceId), req.kennelId!);
      res.json(result);
    } catch (e) {
