...
router.get(
  '/',
  apiRoute({
    method: 'get', path: '/api/breeder/medications', tags: T, secure: true,
    summary: 'All medications for the kennel (active first).',
    responses: { 200: { description: 'ok', schema: z.object({ medications: z.array(z.record(z.string(), z.unknown())) }) } },
  }),
  ah(async (req, res) => {
...
router.post(
  '/',
  apiRoute({
    method: 'post', path: '/api/breeder/medications', tags: T, secure: true,
    summary: 'Add a medication + schedule for an animal.',
    request: {
      body: z.object({
...
router.patch(
  '/:id',
  apiRoute({
    method: 'patch', path: '/api/breeder/medications/{id}', tags: T, secure: true,
...
router.get(
  '/due',
  apiRoute({
    method: 'get', path: '/api/breeder/medications/due', tags: T, secure: true,
    summary: 'Doses due in the next `hours` (default 24, max 72), for the worklist.',
    request: { query: z.object({ hours: z.coerce.number().int().min(1).max(72).optional() }) },
    responses: { 200: { description: 'ok', schema: z.object({ due: z.array(z.record(z.string(), z.unknown())) }) } },
  }),
  ah(async (req, res) => {
...
router.post(
  '/:id/log',
  apiRoute({
    method: 'post', path: '/api/breeder/medications/{id}/log', tags: T, secure: true,
    summary: 'Register a dose as given / skipped / refused.',
    request: {
      params: idParam,
      body: z.object({
        outcome: z.enum(['given', 'skipped', 'refused']).optional(),
        scheduledFor: z.string().optional(),
        note: z.string().optional(),
        photoUrl: z.string().optional(),
      }),
    },
    responses: { 201: { description: 'logged' }, 404: { description: 'not found' } },
  }),
  ah(async (req, res) => {
...
router.get(
  '/:id/compliance',
  apiRoute({
    method: 'get', path: '/api/breeder/medications/{id}/compliance', tags: T, secure: true,
    summary: '30-day compliance report + recent missed doses.',
    request: { params: idParam },
    responses: { 200: { description: 'ok' }, 404: { description: 'not found' } },
  }),
  ah(async (req, res) => {
...
router.post(
  '/sweep-missed',
  apiRoute({
    method: 'post', path: '/api/breeder/medications/sweep-missed', tags: T, secure: true,
    summary: 'Raise a care-inbox exception per medication with an overdue, unlogged dose.',
