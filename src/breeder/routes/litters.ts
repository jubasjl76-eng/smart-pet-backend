...
router.get(
  ...
  apiRoute({
    ...
    responses: {
      200: {
        description: 'ok',
        schema: z.object({ litters: z.array(z.record(z.string(), z.unknown())) }),
      },
    },
  }),
  ah(async (req, res) => {
...
router.post(
  ...
  apiRoute({
    method: 'post',
    path: '/api/breeder/litters/{id}/whelp',
    tags: T,
    secure: true,
...
router.get(
  ...
  apiRoute({
    method: 'get',
    path: '/api/breeder/litters/{id}/puppies',
    tags: T,
    secure: true,
  ...
  ah(async (req, res) => {
    // Growth series across every puppy in the litter (one weight-history query
    // per pup) — routed to the read replica (Phase 21, A11/A12; falls back to
    // the primary when unconfigured).
    ...
    const dam = litter.dam_id
      ? await queryOneReplica<{ adult_weight_kg: number | null }>(
          `SELECT adult_weight_kg FROM animals WHERE id=$1`,
          [litter.dam_id],
        )
      : null;
    const pups = await queryReplica<{
      id: string;
      name: string;
      status: string;
    ...
    const withGrowth = await Promise.all(
      pups.map(async (p) => {
        const w = await queryReplica<{ grams: number; taken_at: string }>(
        ...
        if (dam?.adult_weight_kg && litter.whelped_at && w.length) {
          const ageDays = Math.floor(
            (Date.parse(w[w.length - 1].taken_at) - Date.parse(litter.whelped_at)) / 86_400_000,
          );
...
router.post(
  ...
  apiRoute({
    method: 'post',
    path: '/api/breeder/litters/puppies/{pupId}/weights',
    tags: T,
    secure: true,
...
router.patch(
  ...
  apiRoute({
    method: 'patch',
    path: '/api/breeder/litters/puppies/{pupId}',
    tags: T,
    secure: true,
  ...
  ah(async (req, res) => {
    ...
    const sets: string[] = [];
    const vals: unknown[] = [req.params.pupId, req.kennelId];
    ...
    const row = await queryOne(
      `UPDATE puppies SET ${sets.join(', ')} WHERE id=$1 AND kennel_id=$2 RETURNING *`,
      vals,
    );
...
router.get(
  ...
  apiRoute({
    method: 'get',
    path: '/api/breeder/litters/buyers/list',
    tags: T,
    secure: true,
    ...
    responses: {
      200: {
        description: 'ok',
        schema: z.object({ buyers: z.array(z.record(z.string(), z.unknown())) }),
      },
    },
  }),
  ah(async (req, res) => {
...
router.post(
  ...
  apiRoute({
    method: 'post',
    path: '/api/breeder/litters/buyers',
    tags: T,
    secure: true,
...
router.patch(
  ...
  apiRoute({
    method: 'patch',
    path: '/api/breeder/litters/buyers/{buyerId}',
    tags: T,
    secure: true,
  ...
  ah(async (req, res) => {
    ...
    const sets: string[] = [];
    const vals: unknown[] = [req.params.buyerId, req.kennelId];
    for (const [k, v] of Object.entries(req.body)) {
      const col = camel[k] ?? k;
      if (allowed.includes(col)) {
        vals.push(v);
    ...
    if (!sets.length) return bad(res, 'No updatable fields');
    const row = await queryOne(
      `UPDATE buyers SET ${sets.join(', ')} WHERE id=$1 AND kennel_id=$2 RETURNING *`,
      vals,
    );
    ...
    if (req.body.puppyId) {
      await execute(
        `UPDATE puppies SET status='reserved', buyer_id=$1 WHERE id=$2 AND kennel_id=$3`,
...
router.get(
  ...
  apiRoute({
    method: 'get',
    path: '/api/breeder/litters/puppies/{pupId}/update-pack',
    tags: T,
    secure: true,
  ...
  ah(async (req, res) => {
    ...
    const meds = await query(
      ...
      ORDER BY ml.administered_at DESC LIMIT 20`,
      [req.kennelId],
    );
    res.json({
      puppy: pup,
      weightSeries: weights,
      weightGainGramsPerDay: dailyGainG(
        (weights as Array<{ grams: number; taken_at: string }>).map((w) => ({
          grams: w.grams,
          takenAt: w.taken_at,
        })),
      ),
