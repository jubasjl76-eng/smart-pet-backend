...
import { withLeaderLock } from '../../leaderLock.js';
...
export function normaliseMessage(topic: string, payloadRaw: Buffer | string): RuleEvent[] {
  ...
  let p: any = {};
  try {
    p = JSON.parse(payloadRaw.toString());
  } catch {
    /* non-JSON payload */
  }
  const at = new Date();
  ...
  if (leaf === 'status') {
    ...
    if (typeof p.rssi === 'number')
      events.push({ ...base, type: 'telemetry', metric: 'rssi', value: p.rssi });
    ...
    if (kind === 'dispense' || kind === 'dispense_acked')
      events.push({
        ...base,
        type: 'dispense_acked',
        value: Number(p.seconds ?? p.durationSec ?? 0),
      });
    if (kind === 'door_open' || kind === 'door_opened')
      events.push({ ...base, type: 'door_opened' });
    ...
    if (kind === 'crash') {
      ...
      events.push({
        ...
        meta: {
          ...base.meta,
          reason: cd.reason,
          rawReason: cd.rawReason,
          fw: cd.fw,
          heapFree: cd.heapFree,
          minHeapFree: cd.minHeapFree,
        },
      });
    }
  } else if (leaf === 'location') {
    if (typeof p.battery === 'number')
      events.push({ ...base, type: 'low_battery', value: p.battery, metric: 'battery' });
...
async function onMessage(topic: string, payload: Buffer): Promise<void> {
  ...
  try {
    carrier = JSON.parse(payload.toString());
  } catch {
    /* non-JSON */
  }
...
async function sweepConsumables(): Promise<void> {
  ...
  for (const c of rows) {
    const status = consumableStatus({
      onHand: c.on_hand,
      lowThreshold: c.low_threshold,
      dailyUse: c.daily_use ?? null,
    });
    if (status.level === 'ok') continue;
    await raiseException({
      kennelId: c.kennel_id,
      kind: 'consumable-low',
      severity: status.level === 'low' ? 'warning' : 'critical',
      title: `${c.name} ${status.level === 'out' ? 'out of stock' : 'low'}`,
      detail: status.message,
      dedupKey: `consumable-low:${c.id}`,
      suggestedAction: 'Restock and update the on-hand count',
  ...
  for (const m of meds) {
    const logs = await query<any>(
      `SELECT medication_id, scheduled_for, administered_at, outcome FROM medication_logs
        WHERE medication_id=$1 AND administered_at > NOW() - INTERVAL '3 days'`,
      [m.id],
    );
    const medLogs: MedLog[] = logs.map((l) => ({
      medicationId: l.medication_id,
      scheduledFor: l.scheduled_for ?? l.administered_at,
      outcome: l.outcome,
    }));
    const missed = missedDoses(medRow(m), medLogs, now);
    if (!missed.length) continue;
    await raiseException({
      kennelId: m.kennel_id,
      kind: 'med-missed',
      severity: 'critical',
...
async function detectOffline(): Promise<void> {
  ...
  for (const d of stale) {
    await execute(`UPDATE devices SET status='offline', is_online=false WHERE device_id=$1`, [
      d.device_id,
    ]);
    ...
    await raiseException({
      ...
      severity: 'warning',
      deviceId: d.device_id,
      title: `${d.name ?? d.device_id} offline`,
      detail: `Last seen ${d.last_seen}`,
      suggestedAction: 'Check power and Wi-Fi at the pen',
      dedupKey: `device-offline:${d.device_id}`,
    }).catch(() => {});
  }
}
...
export function startBreederEngine(): void {
  ...
  // Leader lock TTL is shorter than the interval so a slow tick can't hold
  // the lock into the next window and starve every instance's re-election.
  const lockTtlMs = Math.max(1000, intervalMs - 5_000);
  tickTimer = setInterval(() => {
    void withLeaderLock('lock:engine-tick', lockTtlMs, engineTick);
  }, intervalMs);
